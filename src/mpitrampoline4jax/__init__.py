...
def is_configured():
    ...
    return (
        "MPITRAMPOLINE_LIB" in os.environ and
        os.environ.get("JAX_CPU_COLLECTIVES_IMPLEMENTATION") == "mpi" and
        Path(os.environ["MPITRAMPOLINE_LIB"]).exists()
    )
...
__all__ = ["is_configured", "get_library_path"]